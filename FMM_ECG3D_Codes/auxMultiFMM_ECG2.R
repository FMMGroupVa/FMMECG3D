...
fitMultiFMM_ECG<-function(vDataMatrix, annotation,
                          ...
                          plotBasename=NA, plotPath="./", moveBackplotsFolder=TRUE){
  ...
  while(continueBackfitting) {
    ...
    if(any(unusedWave)){
      additionalComponents<-searchAdditionalComponents(vDataMatrix=denseDataMatrix, fittedWaves=fittedWaves, alphaN=alphaN, leadNames=leadNames,
    ...
    if(!"P" %in% rownames(paramsPerLead[[1]])){
      ...
      if("P" %in% rownames(lastChanceP_paramsPerLead[[1]])){
        ...
        additionalComponentsP<-searchAdditionalComponents(vDataMatrix=denseDataMatrix, fittedWaves=fittedWaves, alphaN=alphaN, leadNames=leadNames,
                                                          weightError=weightError, errorWeights=errorWeights, signalWeights=signalWeights,
...
fitMultiFMM_justR2<-function(vDataMatrix, commonOmega=TRUE, weightError=TRUE, showProgress=TRUE, parallelize = FALSE,
                             ...
                             ecgId=NA, beatId=NA, patientClass=NA, patientSuperclass=NA,
                             resultsFile=NA, plotToFile=FALSE, saveResults=FALSE,
                             plotBasename=NA, plotPath=paste(plotPath,"03 Results/",sep="")){
  ...
  r2Fmm<-data.frame(t(sapply(5:nBack, function(x) sapply(multiFmm, function(y) sum(y$Var[1:x])))))
  rownames(r2Fmm)<-paste0("R2_",5:nBack)
...
searchAdditionalComponents<-function(vDataMatrix, fittedWaves, leadNames, alphaN,
  ...
  return(list("paramsPerLead"=paramsPerLead, "extraComponent"=currentIndex+5,
              "fittedWaves"=fittedWaves, "errorWeights"=errorWeights, "totalMSE"=totalMSE))
}
